...
/**
 * @brief 根据key再dict中查找对应得键值对
 * 
 * @param dict 待查找字典
 * @param key  键
 * @return dictEntry* 查找成功返回对应键值对，否则返回NULL
 */
dictEntry* dictSearch(dict* dict, void* key){
        // 判断是否在rehash,并进行相关处理
        bool flag = dict->rehashidx == -1 ? 0 : 1;
        // 计算出待插入键值对的pos
        unsigned int pos = dict->type->hashFunction(key) & dict->ht[flag].sizemask;
        // 取出pos位置的键值对或者键值对链表(注意可能取出来为空)
        dictEntry* head_dictEntry = dict->ht[flag].table[pos];
        // 设置返回得键值对
        dictEntry* ans = NULL;
        // 若链表不为空，遍历键值对链表，查找是否存在该键值对
        while (head_dictEntry != NULL){
            if (head_dictEntry->key == key){
                ans = head_dictEntry;
                break;
            }
        }
...
void dictReplace(dict* dict, void* raw_key, void* raw_val){
    ...
    dictRehash(dict);
    // 查找键是否已经在字典中
    dictEntry* correSpondingDictEntry = dictSearch(dict,raw_key);
    // 如果不为空，则说明已经在字典中,修改对应值即可
    // TODO 这里需要解决value的泛型问题
    correSpondingDictEntry->v.val  = (void*)dict->type->valDup(dict->privdata,raw_val);
    // 如果为空，则插入该键值对
    dictAdd(dict, raw_key, raw_val);
}