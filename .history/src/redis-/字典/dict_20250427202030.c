...
void dictDictEntryInsert (dict* dict, bool flag, void *raw_key, void *raw_val){
    ...
    // 如果为空，直接将待插入键值对，放置该处
    if (head_dictEntry == NULL) *(dict->ht[flag].table) = &new_dictEntry;
    // 如果不为空，将待插入的插入头
    dict->ht[flag].table[pos] = 
}
...
void dictAdd (dict* dict, void *raw_key, void *raw_val){
    ...
    // 计算出待插入键值对的pos
    unsigned int pos = dict->type->hashFunction(raw_key) & dict->ht[1].sizemask;
    // 取出pos位置的键值对或者键值对链表(注意可能取出来为空)
    dictEntry* head_dictEntry = dict->ht[1].table[pos];
    // 如果为空，直接将待插入键值对，放置该处
    dict->ht[1]
}