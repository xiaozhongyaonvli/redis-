...
SDS* sds_grow(SDS* sds, long ch_size, const char* ch){
    ...
    SDS* new_sds;
    if (memory_size < 1)         new_sds = sds_init(2*memory_size*1024*1024*8+1); // 注意单独的空字符空间 else  //如果修改后SDS长度大于等于1MB，那么程序将分配 修改后+1MB+1byte
    else if (memory_size >= 1)   new_sds = sds_init((memory_size+1)*1024*1024*8+1);
    // 将修改后的字节数组赋值给新的sds
    memcpy(new_sds,ch,ch_size);
    delete sds;      // 删除旧的sds，防止内存泄漏
